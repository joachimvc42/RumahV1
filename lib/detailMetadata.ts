...
const PREFIX: Record<'en' | 'fr' | 'es' | 'id', string> = { en: '', fr: '/fr', es: '/es', id: '/id' };
  ...
  if (inv) {
    ...
    if (asset) {
      const noun = inv.asset_type === 'property'
        ? (locale === 'id' ? 'vila' : 'villa')
        : (locale === 'fr' ? 'terrain' : locale === 'es' ? 'terreno' : locale === 'id' ? 'tanah' : 'land');
      const prep = locale === 'fr' ? 'à' : locale === 'es' ? 'en' : locale === 'id' ? 'di' : 'in';
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        id: `${BASE_URL}/id/opportunities/${id}`,
