...
type Filters = {
  ...
  furnished: boolean; wifi: boolean; parking: boolean; privateSpace: boolean;
};
...
function RentalCard({ rental }: { rental: RentalRow }) {
  ...
  const waMsg = encodeURIComponent(`Hello, I'm interested in the rental: ${p?.title}`);

  return (
    <div style={C.card}>
      {/* IMAGE — fixed height 220px, images stacked via opacity, arrows never move */}
      <div style={C.imgWrap}>
        ...
        {images.length > 1 && <div style={C.imgCount}>📷 {images.length}</div>}
        {images.length > 1 && (
          <>
...
export default function RentalsPage() {
  ...
  const [filters, setFilters] = useState<Filters>({
    ...
    furnished: false, wifi: false, parking: false, privateSpace: false,
  });
  ...
  useEffect(() => {
    supabase.from('long_term_rentals')
      .select(`id, min_duration_months, max_duration_months, monthly_price_idr, upfront_months, legal_checked, available_from,
        properties (id, title, location, bedrooms, bathrooms, pool, garden, furnished, aircon, wifi, parking, private_space, images, status)`)
  ...
  const reset = () => setFilters({ location: '', minBeds: '', maxPrice: '', pool: false, garden: false, aircon: false, furnished: false, wifi: false, parking: false, privateSpace: false });
    ...
    <main style={P.page}>
      ...
      <div style={P.layout}>
        <aside style={P.sidebar}>
          ...
          {([
            ['pool','🏊 Pool'],['garden','🌳 Garden'],['aircon','❄️ Air conditioning'],
            ['furnished','🛋️ Furnished'],['wifi','📶 WiFi'],['parking','🚗 Parking'],
        ...
        <div style={P.main}>
          ...
          <div style={P.resultRow}>
            <p style={{ fontSize:15, color:'#6b7280', margin:0 }}>{filtered.length} propert{filtered.length !== 1 ? 'ies' : 'y'} available</p>
            {Object.values(filters).some(v => v !== '' && v !== false) && (
              <button onClick={reset} style={{ fontSize:14, fontWeight:600, color:'#2563eb', background:'none', border:'none', cursor:'pointer' }}>Reset filters</button>
            )}
          </div>

          {filtered.length === 0 ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:16, padding:60, background:'#f9fafb', borderRadius:16 }}>
              <p style={{ color:'#6b7280', fontSize:16 }}>No properties match your criteria.</p>
              <button onClick={reset} style={{ padding:'12px 24px', background:'#2563eb', color:'#fff', border:'none', borderRadius:10, fontSize:15, fontWeight:600, cursor:'pointer' }}>Reset filters</button>
            </div>
          ) : (
...
// Card styles — images stacked at fixed height, arrows always centred
const C: { [k: string]: React.CSSProperties } = {
  ...
  badgeVerified: { position:'absolute', top:12, left:12, background:'#166534', color:'#fff', fontSize:12, fontWeight:700, padding:'5px 11px', borderRadius:6, letterSpacing:'0.04em' },
  imgCount: { position:'absolute', top:12, right:12, background:'rgba(0,0,0,0.55)', color:'#fff', fontSize:12, fontWeight:600, padding:'5px 10px', borderRadius:6 },
  // Arrows: top:50% + transform so they NEVER move regardless of image content
  ...
  priceRow: { display:'flex', alignItems:'baseline', gap:6, marginBottom:3 },
  price: { fontSize:22, fontWeight:800, color:'#111827' },
  per: { fontSize:14, color:'#6b7280' },
  dur: { fontSize:13, color:'#9ca3af', margin:0 },
  waBtn: { display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'12px 16px', background:'#25d366', color:'#fff', textDecoration:'none', fontWeight:700, fontSize:14, margin:'10px 16px 16px', borderRadius:10, borderTop:'1px solid #e5e7eb' },
};
...
const P: { [k: string]: React.CSSProperties } = {
  ...
  sHead: { fontSize:11, fontWeight:800, letterSpacing:'0.13em', color:'#2563eb', margin:'0 0 14px', textTransform:'uppercase' },
  ...
  searchBar: { display:'flex', background:'#fff', borderRadius:14, border:'1px solid #e5e7eb', overflow:'hidden', marginBottom:24, boxShadow:'0 2px 12px rgba(15,23,42,0.06)' },
