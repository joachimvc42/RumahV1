...
export default function EditRentalPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase.from('long_term_rentals').select('*, properties (*)').eq('id', id).single();
      ...
      if (data.properties) {
        ...
        setPropertyId(p.id);
        setTitle(p.title || ''); setLocation(p.location || ''); setDescription(p.description || '');
        setBedrooms(String(p.bedrooms || '')); setBathrooms(String(p.bathrooms || ''));
        setBuiltArea(String(p.built_area || '')); setLandArea(String(p.land_area || ''));
        setPool(p.pool || false); setGarden(p.garden || false);
        setFurnished(p.furnished ?? true); setAircon(p.aircon ?? true);
        setWifi(p.wifi ?? true); setParking(p.parking || false);
        setStatus((p.status as any) || 'draft');
        setMediaItems(urlsToMediaItems(p.images || [], p.videos || []));
      }
      setLoading(false);
    };
    load();
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try { await supabase.from('long_term_rentals').delete().eq('id', id); router.push('/admin'); }
      ...
      <form onSubmit={handleSave} style={s.form}>
        <section style={s.section}>
          ...
          <div style={s.amenities}>
            {(['pool','garden','furnished','aircon','wifi','parking'] as const).map(key => {
