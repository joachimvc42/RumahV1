...
export default async function OpportunitiesPageES() {
  const items = await getInvestments('es');
  const itemListJsonLd = investmentsItemListJsonLd(items, 'es');
  return (
    <>
      <script
      ...
      />
      <InvestmentsClient locale="es" initialItems={items} />
    </>
  );
}
